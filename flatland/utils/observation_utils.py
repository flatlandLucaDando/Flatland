...
def _split_node_into_feature_groups_new(node):
    ...
    data[4] = node.dist_own_stations
    data[5] = node.own_stations_index
    data[6] = node.time_own_station
    data[7] = node.dist_other_station_encountered
    data[8] = node.time_other_station_encountered
...
def _split_subtree_into_feature_groups(node, current_tree_depth: int, max_tree_depth: int):
    if node == -np.inf:
        ...
        return [-np.inf] * num_remaining_nodes * 9, [-np.inf] * num_remaining_nodes * 5
