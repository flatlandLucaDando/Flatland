...
class TreeTimetableObservation(ObservationBuilder):
    """
    TreeObsForRailEnv object.
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        #10:
            if a station in the timetable_real_time lies on the exploration of the branch the distance in cell from that station is stored
        
        #11:
            if a station present in the timetable_real_time lies on the exploration of the branch the index of the station (first to be reached, second etc etc)
            is stored 
            
        #12:
            if a station present in the timetable // // the time at which the station should be reached is stored
            
        #13:
            if a station in the timetable_real_time for the other_agent_0 lies on the exploration of the branch the distance in cell from that station is stored
        
        #14:
            if a station present in the timetable_real_time for the other_agent_0 lies on the exploration of the branch the index of the station (first to be reached, second etc etc)
            is stored 
        ...
        for i, branch_direction in enumerate([(orientation + i) % 4 for i in range(-1, 3)]):
            ...
            # ---------------------------------------------------------------------------------------
            # ---------------------------------------------------------------------------------------
            # Se vuoi sistemare la REVERSE agisci qua
            """elif i == 3: # allora creo il branch e esploro il branch
                new_cell = get_new_position(agent_virtual_position, i)

                branch_observation, branch_visited = \
                    self._explore_branch(handle, new_cell, i, 1, 1)
                root_node_observation.childs[self.tree_explored_actions_char[i]] = branch_observation
    ...
    def _explore_branch(self, handle, position, direction, tot_dist, depth):
        ...
        real_time_station_capacity = np.inf
        ...
        rail, optionals = self.env.rail_generator(
        ...
        agents_in_station = [0]*len(timetable[handle][0])
        ...
        while exploring:
            ...
            if position in self.env.cell_interrupted:
                        ...
                        if position == self.env.next_station_to_reach[handle][i_station].rails:  
                            ...
                            for i_station in range(len(timetable[handle][0])):
                                ...
                                if timetable[handle][0][i_station].position == position:
                                    time_difference = abs(timetable[handle][1][i_station] - time_now)
                                    if min_time > time_difference:
                                        index_of_my_station = i_station
                            
                            time_difference = abs(timetable[handle][1][index_of_my_station] - time_now)
                            time_own_station = time_difference       # save the time at which i have to reach it
                    ...
                    for j_agent in range(self.env.get_num_agents()):
                        if i_agent != handle and j_agent != handle and self.env.next_station_to_reach[i_agent] != [] and self.env.next_station_to_reach[j_agent] != []:
                            if position in self.env.next_station_to_reach[i_agent][0].rails:
                                ...
                                time_now = self.env._elapsed_steps 
                                difference_time = abs(timetable[i_agent][1][index_of_my_station] - time_now)
                                ...
                                capacity_of_station = timetable[i_agent][0][index_of_my_station].capacity
                                
                                train_velocity = timetable[i_agent][4].maximum_velocity
                                
                                if difference_time <= min_dist * (1/train_velocity):
                                    agents_in_station[index_of_my_station] += 1
                                
                                real_time_station_capacity = capacity_of_station - agents_in_station
        ...
        node = NodeNew( dist_other_agent_encountered=dist_other_agent_encountered,
                        ...
                        real_time_station_capacity = real_time_station_capacity,
    ...
    @staticmethod
    def print_node_features(node: Node, label, indent):
        print(indent, "Direction ", label, ": ", node.dist_other_agent_encountered, ", ",
              node.dist_potential_conflict, ", ", node.dist_unusable_switch, ", ", node.dist_to_next_branch, ", ", node.num_agents_same_direction, ", ", node.num_agents_opposite_direction,
              ", ", node.num_agents_malfunctioning, ", ", node.speed_min_fractional, ", ",
              node.num_agents_ready_to_depart, ",", node.station_positions, ",", node.station_index, ",",
              node.time_at_which_reach_station)
